...
function ThreeBackground() {
  ...
  useEffect(() => {
    const initThree = async () => {
      ...
      const segments  = isAndroid ? 32 : isLowEnd ? 64 : 128
      ...
      renderer.setPixelRatio(Math.min(window.devicePixelRatio, isAndroid ? 1 : 1.5))
      ...
      const fragmentShaderAndroid = `
        ...
        void main() {
          ...
          vec3 col = mix(vec3(0.40, 0.12, 0.70), vec3(0.60, 0.35, 0.85), NdV);
          col = mix(col, vec3(0.18, 0.03, 0.35), 0.3);
          col += vec3(0.40, 0.12, 0.70) * pulse * 0.1;
          vec3 color = mix(col, vec3(0.75, 0.70, 0.90), fresnel * 0.6);
          gl_FragColor = vec4(clamp(color, 0.0, 1.0), mix(0.95, 0.60, fresnel * 0.4));
        }
      `
      ...
      const material = new THREE.ShaderMaterial({
        ...
        vertexShader,
        fragmentShader: isAndroid ? fragmentShaderAndroid : fragmentShaderFull,
        transparent: true,
      ...
      // ── Context lost/restored — THE key Android fix ──
      const handleContextLost = (e) => {
        e.preventDefault()          // required or context will never restore
        isContextLost = true
      ...
      // ── Visibility change — Android suspends RAF when tab hidden ──
      ...
      const handleScroll = () => {
        ...
        targetScale = isAndroid ? 1.0 + progress * 0.3 : 1.0 + progress * 2.0
      }
