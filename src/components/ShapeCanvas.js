...
const SCRIPT_SRC = 'https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js'
  ...
  return new Promise((resolve) => {
    ...
    if (document.querySelector(`script[src="${SCRIPT_SRC}"]`)) {
      const wait = setInterval(() => {
        if (window.THREE) { clearInterval(wait); resolve(window.THREE) }
      }, 50)
      return
    }
...
export default function ShapeCanvas({ shape, solidColor, emissive }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Wide and short — matches the new aspect ratio
      const RW = 800
      const RH = 220
      ...
      // Very wide FOV for a wide-short canvas
      ...
      // Main solid — slightly left of centre
      ...
      // Right solid — pushed far right, vertically centred
      ...
      // Left solid — pushed far left, slightly low
